  ...
  describe('Marketplace Pause contract', async () => {
    ...
    describe('Marketplace Test modifier whenNotPaused', async () => {
      ...
      it('should not be able to transfer service ownership', async () => {
        await truffleAssert.reverts(contract.createService(utils.asciiToHex(sid), price, { from: developer }))
      })
      it('should not be able to transfer service ownership', async () => {
      ...
      it('should not be able to create a service version', async () => {
        await truffleAssert.reverts(contract.createServiceVersion(0, version.hash, utils.asciiToHex(version.url), { from: developer }))
      })
    })
  })
})
